...
class Dataset:
    ...
    def random_get_rays_at(self, img_idx, batch_size, importance_sample, pose = None):
        ...
        if self.use_normal:
            ...
            normal_mask_sample = self.normal_mask[img_idx][(pixels_y, pixels_x)].cuda()
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color, mask], dim=-1).cuda(), pixels_x, pixels_y, normal_sample, planes_sample, subplanes_sample, normal_mask_sample    # batch_size, 10
    ...
    def score_pixels_ncc(self, idx, pts_world, normals_world, pixels_coords_vu, reso_level = 1.0, _debug = False):
        ...
        valid_flag = True
        for ne in idx_neighbors:
            if ne < 0 or ne > self.n_images-1:
                valid_flag = False
