...
def evaluate_3D_mesh(path_mesh_pred, scene_name, dir_dataset = './dataset/indoor',
    ...
    '''
    path_intrin = f'{dir_dataset}/{scene_name}/intrinsics.txt'
    target_img_size = (640, 480)
    dir_scan = f'{dir_dataset}/{scene_name}'
    dir_poses = f'{dir_scan}/pose'
    dir_images = f'{dir_scan}/image'
    
    path_mesh_gt = f'{dir_dataset}/{scene_name}/point_cloud_openMVS.ply'
    path_mesh_net = f'{path_mesh_pred}/exp_{scene_name}/meshes/00160000_reso1024_{scene_name}_world.ply'
    ...
    path_mesh_pred_clean_bbox = os.path.join(path_mesh_pred, 'clean_bbox.obj')
    path_mesh_pred_clean_bbox_faces = os.path.join(path_mesh_pred, 'clean_frustum.obj')
    # path_mesh_pred_clean_bbox_faces_mask = IOUtils.add_file_name_suffix(path_mesh_pred, '_clean_bbox_faces_mask')

    GeoUtils.clean_mesh_points_outside_bbox(path_mesh_pred_clean_bbox, path_mesh_net, path_mesh_gt,
                                                scale_bbox=1.1,
                                                check_existence = check_existence)
