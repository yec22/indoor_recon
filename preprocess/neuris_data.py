...
def prepare_neuris_data_from_private_data(dir_neus, size_img = (6016, 4016),
    ...
    if b_pred_normal:
        predict_normal(dir_neus, normal_method, size_img)
...
def predict_normal(dir_neus, normal_method = 'snu', img_size = (640, 480)):
    ...
    if normal_method == 'snu':
        ...
        os.system(f'python test.py --pretrained scannet --architecture BN --input_width {img_size[0]} --input_height {img_size[1]} --imgs_dir {dir_neus}/image/')
        
        pred_save_dir = os.path.join(f'{dir_neus}/pred_normal/')
        if not os.path.exists(pred_save_dir):
            os.makedirs(pred_save_dir)

        for name in sorted(os.listdir(f'{dir_neus}/pred_normal_png/')):
            norm_data = (cv2.imread(os.path.join(f'{dir_neus}/pred_normal_png/', name)) / 255.0 - 0.5) * 2
